...
class EvalCallback(EventCallback):
    ...
    def __init__(
        self,
        eval_env: Union[gym.Env, BaseVecEnv],
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            if self.log_path is not None:
                self.evaluations_timesteps.append(self.num_time_steps)
            ...
            self.logger.record(
                "time/total_timesteps", self.num_time_steps, exclude="tensorboard"
            )
            self.logger.dump(self.num_time_steps)

...
class RatioSelfplayStrategy(SelfplayStrategy):
    ...
    def get_plist(self):
        if self.history_cnt == 1:
            return [1]
        temp_plist = np.logspace(0, self.history_cnt - 1, self.history_cnt, endpoint=True, base=1.5)
...
class OnlyLatestSelfplayStrategy(SelfplayStrategy):
    ...
    def save_new_one(self, least_win_rate):
        if sum(np.array(self.play_list[-1]) == -1) >= least_win_rate * (len(self.play_list[-1]) + 1) and len(
                self.play_list[-1]) >= (self.max_play_num - 10):
            return True
    ...
    def cut_overflow(self):
        for index in range(len(self.play_list)):
            if len(self.play_list[index]) > self.max_play_num:
                self.play_list[index] = self.play_list[index][(-1) * self.max_play_num:]
    ...
    def get_enemy_play_dict(self):
        win_num_list, tie_num_list, lose_num_list, play_num_list = self.get_info_list(["win", "tie", "lose", "play"])
        return {"win_num_list": list(win_num_list), "tie_num_list": list(tie_num_list),
                "lose_num_list": list(lose_num_list), "play_num_list": list(play_num_list)}
...
class WeightSelfplayStrategy(SelfplayStrategy):
    ...
    def get_plist(self):
        ...
        temp_plist[:(-1 * self.recent_num)] = np.exp(self.qlist) / sum(np.exp(self.qlist)) * (1 - self.recent_weight)
    ...
    def update_weight(self, enemy_loses):
        if self.history_cnt < self.recent_num + 2:
            return
        lose_enemy_ids = np.array(self.enemy_ids)[enemy_loses]  # 输了的enemy_ids,进行更新,其中可能有重复的enemy_id
        for enemy_id in lose_enemy_ids:
            if enemy_id <= len(self.qlist) - 1:
                divide_num = len(self.qlist) * np.exp(self.qlist[enemy_id]) / sum(np.exp(self.qlist))
...
class WinRateSelfplayStrategy(SelfplayStrategy):
    def __init__(self, all_args, nenvs, exist_enemy_num):
        ...
        self.max_play_num = all_args.max_play_num
        self.play_list = []  # 在该list中，每个对手维护一个长度不超过max_play_num的列表，1为该对手获胜, 0为平, -1为我方获胜
        self.recent_list = []
    ...
    def get_plist(self):
        ...
        win_num_list, tie_num_list, play_num_list = self.get_info_list(["win", "tie", "play"])
        win_rate_list = (np.array(win_num_list) + 0.5 * np.array(tie_num_list) + 0.5) / (np.array(play_num_list) + 1)
    ...
    def update_play_list(self, win_enemy_ids, tie_enemy_ids, lose_enemy_ids):
        if self.stage == 2:
            ...
            self.recent_list += ([1] * win_enemy_num + [0] * tie_enemy_num + [-1] * lose_enemy_num)
        for win_enemy_id in win_enemy_ids:
            self.play_list[win_enemy_id].append(1)
        for tie_enemy_id in tie_enemy_ids:
            self.play_list[tie_enemy_id].append(0)
    ...
    def update_win_info(self, data):
        win_enemy_ids, tie_enemy_ids, lose_enemy_ids = data["win_enemy_ids"], data["tie_enemy_ids"], data[
            "lose_enemy_ids"]
        self.update_play_list(win_enemy_ids, tie_enemy_ids, lose_enemy_ids)
    ...
    def save_new_one(self, least_win_rate):
        ...
        if self.stage == 2:
            if sum(np.array(self.recent_list) == -1) >= self.stage2_least_win_rate * (
                    len(self.recent_list) + 1) and len(self.recent_list) >= (self.recent_list_max_len - 10):
                self.stage = 1
                self.recent_list = []
                return True
        return False
    ...
    def get_plist(self):
        ...
        if self.max_enemy_num != -1:
            if self.history_cnt - self.exist_enemy_num > self.max_enemy_num:
                mask_index = np.array(list(range(self.exist_enemy_num, self.history_cnt - self.max_enemy_num)))
                zero_vec = np.zeros(self.history_cnt - self.exist_enemy_num - self.max_enemy_num)
...
class WeightExistEnemySelfplayStrategy(ExistEnemySelfplayStrategy):
    ...
    def get_final_plist(self, f_hard, f_var):
        ...
        if self.stage == 1:
            ...
            win_rate_list = win_rate_list / (sum(win_rate_list) + 1e-8) * (1 - self.latest_weight)
            ...
            if self.newest_pos != -1:
                ...
                win_rate_list[index_without_newest] /= sum(win_rate_list[index_without_newest])
